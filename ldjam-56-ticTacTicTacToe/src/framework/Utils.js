...
export default class Utils {
   ...
   * @returns {Array}
   */
  static except(origin, other) {
    const result = [];

    for (let i = 0; i < origin.length; ++i)
      if (!Utils.contains(other, origin[i])) result.push(origin[i]);
   ...
   * @returns {Object}
   */
  static getRandomEl(arr, debug) {
    const index = Utils.getRandom(0, arr.length - 1, debug);
    return arr[index];
  }
