...
export default class Utils {
  ...
  static buildMatrix(value, width, height) {
    const res = [];
    for (let i = 0; i < height; ++i) {
      const row = Utils.buildArray(width, value);
      res.push(row);
    }
    return res;
  }

  static copyMatrix(origin) {
    const res = [];
    for (let i = 0; i < origin.length; ++i) {
      const row = Utils.copyArray(origin[i]);
      res.push(row);
    }
    return res;
  }

  static getStrongNeighbours(arr, i, j) {
    const result = [];
    for (let y = -1; y <= 1; ++y)
      for (let x = -1; x <= 1; ++x) {
        if (x != 0 && y != 0) continue;
